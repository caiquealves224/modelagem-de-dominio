...
export default class ProgressoCurso extends Entidade<ProgressoCurso, ProgressoCursoProps> {
    ...
    constructor(
        ...
        this.aulaSelecionada =
            this.aulas.find((aula) => aula.id.valor === props.aulaSelecionadaId) ?? this.aulas[0]
    ...
    riscoDeFraude(): number {
        ...
        const total = this.aulas.reduce((total, aulaAtual, i) => {
            ...
            const intervaloReal = Math.abs(dataAtual.getTime() - dataDoProximo.getTime())
    ...
    registrar(observador: ObservadorEventoDominio<CursoConcluido>): ProgressoCurso {
    ...
    get duracaoTotal(): Duracao {
        return this.aulas.reduce((total, aula) => total.somar(aula.duracao), new Duracao())
    }
    ...
    private notificarConclusao() {
        ...
        this.observadores.forEach(obs => obs.eventoOcorreu(evento))
    }
}
